...
@interface HFSwipeTableInputOverlay : UIView

@property (nonatomic, weak) HFSwipeTableCell * currentCell;

@end

@implementation HFSwipeTableInputOverlay
...
@interface HFSwipeButtonsView : UIView

@property (nonatomic, weak) HFSwipeTableCell * cell;
@property (nonatomic, strong) UIColor * backgroundColorCopy;

@end

@implementation HFSwipeButtonsView
{
    NSArray * _buttons;
    UIView * _container;
    BOOL _fromLeft;
    ...
    HFSwipeDirection _direction;
    HFSwipeExpansionLayout _expansionLayout;
...
#pragma mark Layout
- (instancetype)initWithButtons:(NSArray *)buttonsArray direction:(HFSwipeDirection)direction swipeSettings:(HFSwipeSettings *)settings safeInset:(CGFloat)safeInset {
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, containerWidth + safeInset, maxSize.height)]) {
        _fromLeft = direction == HFSwipeDirectionLeftToRight;
        ...
        for (UIView *button in _buttons) {
            if ([button isKindOfClass:[UIButton class]]) {
                ...
                [btn removeTarget:nil action:@selector(HFButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
                // Remove all targets to avoid problems with reused buttons among many cells
                [btn addTarget:self action:@selector(HFButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
            }
            if (!settings.allowsButtonsWithDifferentWidth) {
        ...
        if (safeInset > 0 && settings.expandLastButtonBySafeAreaInsets && _buttons.count > 0) {
            UIView *notchButton = _direction == HFSwipeDirectionRightToLeft ? [_buttons lastObject] : [_buttons firstObject];
...
- (void)dealloc {
    for (UIView *button in _buttons) {
        if ([button isKindOfClass:[UIButton class]]) {
            [(UIButton *)button removeTarget:self action:@selector(HFButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        }
    }
}
...
- (void)setSafeInset:(CGFloat)safeInset extendEdgeButton:(BOOL)extendEdgeButton isRTL:(BOOL)isRTL {
    ...
    if (diff != 0) {
        ...
        if (extendEdgeButton) {
            UIView *edgeButton = _direction == HFSwipeDirectionRightToLeft ? [_buttons lastObject] : [_buttons firstObject];
        ...
        if (_direction == HFSwipeDirectionLeftToRight) {
...
- (void)adjustContentEdge:(UIView *)view edgeDelta:(CGFloat)delta {
    if ([view isKindOfClass:[UIButton class]]) {
        ...
        if (_direction == HFSwipeDirectionRightToLeft) {
...
- (CGRect)expansionBackgroundRect:(UIView *)button {
    // extra size to avoid expansion background size issue on iOS 7.0
    CGFloat extra = 100.0f;
    if (_fromLeft) {
...
- (void)expandToOffset:(CGFloat)offset settings:(HFSwipeExpansionSettings *)settings {
    ...
    if (!_expandedButton) {
        ...
        [UIView animateWithDuration: duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if (self->_expansionLayout == HFSwipeExpansionLayoutCenter) {
            ...
            else if (self->_expansionLayout == HFSwipeExpansionLayoutNone) {
                [self->_expandedButton.superview bringSubviewToFront:self->_expandedButton];
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    if ([sender respondsToSelector:@selector(callHFSwipeConvenienceCallback:)]) {
        // call convenience block callback if exits (usage of HFSwipeButton class is not compulsory)
        autoHide = [sender performSelector:@selector(callHFSwipeConvenienceCallback:) withObject:_cell];
    }
#pragma clang diagnostic pop
    ...
    if (_cell.delegate && [_cell.delegate respondsToSelector:@selector(swipeTableCell:tappedButtonAtIndex:direction:fromExpansion:)]) {
        ...
        autoHide |= [_cell.delegate swipeTableCell:_cell tappedButtonAtIndex:index direction:_fromLeft ? HFSwipeDirectionLeftToRight : HFSwipeDirectionRightToLeft fromExpansion:fromExpansion];
    }
...
// button listener
- (void)HFButtonClicked:(id)sender {
...
- (void)transition:(HFSwipeTransition)mode percent:(CGFloat)t {
    switch (mode) {
        case HFSwipeTransitionStatic: [self transitionStatic:t]; break;
        case HFSwipeTransitionDrag: [self transitionDrag:t]; break;
        case HFSwipeTransitionClipCenter: [self transitionClip:t]; break;
        case HFSwipeTransitionBorder: [self transtitionFloatBorder:t]; break;
        case HFSwipeTransitionRotate3D: [self transition3D:t]; break;
    }
...
@implementation HFSwipeSettings

- (instancetype)init {
    if (self = [super init]) {
        self.transition = HFSwipeTransitionBorder;
        self.threshold = 0.5;
        self.offset = 0;
        ...
        self.showAnimation = [[HFSwipeAnimation alloc] init];
        self.hideAnimation = [[HFSwipeAnimation alloc] init];
        self.stretchAnimation = [[HFSwipeAnimation alloc] init];
    }
    return self;
}
...
@implementation HFSwipeExpansionSettings
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.triggerAnimation = [[HFSwipeAnimation alloc] init];
    }
    return self;
}
...
@interface HFSwipeAnimationData : NSObject
...
@property (nonatomic, strong) HFSwipeAnimation * animation;

@end

@implementation HFSwipeAnimationData

@end

#pragma mark Easing Functions and HFSwipeAnimation

static inline CGFloat HFEaseLinear(CGFloat t, CGFloat b, CGFloat c) {
    return c*t + b;
}

static inline CGFloat HFEaseInQuad(CGFloat t, CGFloat b, CGFloat c) {
    return c*t*t + b;
}

static inline CGFloat HFEaseOutQuad(CGFloat t, CGFloat b, CGFloat c) {
    return -c*t*(t-2) + b;
}

static inline CGFloat HFEaseInOutQuad(CGFloat t, CGFloat b, CGFloat c) {
    if ((t*=2) < 1) return c/2*t*t + b;
    --t;
    return -c/2 * (t*(t-2) - 1) + b;
}

static inline CGFloat HFEaseInCubic(CGFloat t, CGFloat b, CGFloat c) {
    return c*t*t*t + b;
}

static inline CGFloat HFEaseOutCubic(CGFloat t, CGFloat b, CGFloat c) {
    --t;
    return c*(t*t*t + 1) + b;
}

static inline CGFloat HFEaseInOutCubic(CGFloat t, CGFloat b, CGFloat c) {
    if ((t*=2) < 1) return c/2*t*t*t + b;
    t-=2;
    return c/2*(t*t*t + 2) + b;
}

static inline CGFloat HFEaseOutBounce(CGFloat t, CGFloat b, CGFloat c) {
    if (t < (1/2.75)) {
...
static inline CGFloat HFEaseInBounce(CGFloat t, CGFloat b, CGFloat c) {
    return c - HFEaseOutBounce (1.0 -t, 0, c) + b;
};

static inline CGFloat HFEaseInOutBounce(CGFloat t, CGFloat b, CGFloat c) {
    if (t < 0.5) return HFEaseInBounce (t*2, 0, c) * .5 + b;
    return HFEaseOutBounce (1.0 - t*2, 0, c) * .5 + c*.5 + b;
};

@implementation HFSwipeAnimation

- (instancetype)init {
    if (self = [super init]) {
        _duration = 0.3;
        _easingFunction = HFSwipeEasingFunctionCubicOut;
    }
    return self;
}
...
- (CGFloat)value:(CGFloat)elapsed duration:(CGFloat)duration from:(CGFloat)from to:(CGFloat)to {
    ...
    switch (_easingFunction) {
        case HFSwipeEasingFunctionLinear: easingFunction = HFEaseLinear;break;
        case HFSwipeEasingFunctionQuadIn: easingFunction = HFEaseInQuad;break;
        case HFSwipeEasingFunctionQuadOut: easingFunction = HFEaseOutQuad;break;
        case HFSwipeEasingFunctionQuadInOut: easingFunction = HFEaseInOutQuad;break;
        case HFSwipeEasingFunctionCubicIn: easingFunction = HFEaseInCubic;break;
        default:
        case HFSwipeEasingFunctionCubicOut: easingFunction = HFEaseOutCubic;break;
        case HFSwipeEasingFunctionCubicInOut: easingFunction = HFEaseInOutCubic;break;
        case HFSwipeEasingFunctionBounceIn: easingFunction = HFEaseInBounce;break;
        case HFSwipeEasingFunctionBounceOut: easingFunction = HFEaseOutBounce;break;
        case HFSwipeEasingFunctionBounceInOut: easingFunction = HFEaseInOutBounce;break;
    }
...
#pragma mark HFSwipeTableCell Implementation

@implementation HFSwipeTableCell
{
    ...
    HFSwipeButtonsView * _leftView;
    HFSwipeButtonsView * _rightView;
    bool _allowSwipeRightToLeft;
    bool _allowSwipeLeftToRight;
    __weak HFSwipeButtonsView * _activeExpansion;

    HFSwipeTableInputOverlay * _tableInputOverlay;
    ...
    HFSwipeAnimationData * _animationData;
    void (^_animationCompletion)(BOOL finished);
    CADisplayLink * _displayLink;
    HFSwipeState _firstSwipeState;
}
...
- (void)initViews:(BOOL)cleanButtons {
    if (cleanButtons) {
        ...
        _leftSwipeSettings = [[HFSwipeSettings alloc] init];
        _rightSwipeSettings = [[HFSwipeSettings alloc] init];
        _leftExpansion = [[HFSwipeExpansionSettings alloc] init];
        _rightExpansion = [[HFSwipeExpansionSettings alloc] init];
    }
    _animationData = [[HFSwipeAnimationData alloc] init];
...
- (void)fetchButtonsIfNeeded {
    if (_leftButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _leftButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:HFSwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings expansionSettings:_leftExpansion];
    }
    if (_rightButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _rightButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:HFSwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings expansionSettings:_rightExpansion];
    }
}
...
- (void)createSwipeViewIfNeeded {
    ...
    if (!_leftView && _leftButtons.count > 0) {
        ...
        _leftView = [[HFSwipeButtonsView alloc] initWithButtons:_leftButtons direction:HFSwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings safeInset:safeInsets.left];
        _leftView.cell = self;
    ...
    if (!_rightView && _rightButtons.count > 0) {
        ...
        _rightView = [[HFSwipeButtonsView alloc] initWithButtons:_rightButtons direction:HFSwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings safeInset:safeInsets.right];
...
- (void)showSwipeOverlayIfNeeded {
    ...
    if (!_allowsMultipleSwipe) {
        ...
        _tableInputOverlay = [[HFSwipeTableInputOverlay alloc] initWithFrame:table.bounds];
...
- (void)prepareForReuse {
    ...
    [self cleanViews];
    if (_swipeState != HFSwipeStateNone) {
        _triggerStateChanges = YES;
        [self updateState:HFSwipeStateNone];
    }
...
- (void)setAccesoryViewsHidden:(BOOL)hidden {
    if (@available(iOS 12, *)) {
        ...
        // See: https://github.com/MortimerGoro/HFSwipeTableCell/issues/337
        if (hidden) {
...
- (void)updateState:(HFSwipeState)newState {
...
- (void)setSwipeOffset:(CGFloat)newOffset {
    ...
    HFSwipeButtonsView * activeButtons = sign < 0 ? _rightView : _leftView;
    HFSwipeSettings * activeSettings = sign < 0 ? _rightSwipeSettings : _leftSwipeSettings;
    ...
    HFSwipeButtonsView* but[2] = {_leftView, _rightView};
    HFSwipeSettings* settings[2] = {_leftSwipeSettings, _rightSwipeSettings};
    HFSwipeExpansionSettings * expansions[2] = {_leftExpansion, _rightExpansion};
    
    for (int i = 0; i< 2; ++i) {
        HFSwipeButtonsView * view = but[i];
        if (!view) continue;
        ...
        if (expand) {
            ...
            [self updateState:i ? HFSwipeStateExpandingRightToLeft : HFSwipeStateExpandingLeftToRight];
        }
        else {
            ...
            [self updateState:i ? HFSwipeStateSwipingRightToLeft : HFSwipeStateSwipingLeftToRight];
        }
    }
}

- (void)hideSwipeAnimated:(BOOL)animated completion:(void(^)(BOOL finished))completion {
    HFSwipeAnimation * animation = animated ? (_swipeOffset > 0 ? _leftSwipeSettings.hideAnimation: _rightSwipeSettings.hideAnimation) : nil;
...
- (void)showSwipe:(HFSwipeDirection)direction animated:(BOOL)animated completion:(void(^)(BOOL finished))completion {
    [self createSwipeViewIfNeeded];
    _allowSwipeLeftToRight = _leftButtons.count > 0;
    _allowSwipeRightToLeft = _rightButtons.count > 0;
    UIView * buttonsView = direction == HFSwipeDirectionLeftToRight ? _leftView : _rightView;
    
    if (buttonsView) {
        CGFloat s = direction == HFSwipeDirectionLeftToRight ? 1.0 : -1.0;
        HFSwipeAnimation * animation = animated ? (direction == HFSwipeDirectionLeftToRight ? _leftSwipeSettings.showAnimation : _rightSwipeSettings.showAnimation) : nil;
...
- (void)expandSwipe:(HFSwipeDirection)direction animated:(BOOL)animated {
    CGFloat s = direction == HFSwipeDirectionLeftToRight ? 1.0 : -1.0;
    HFSwipeExpansionSettings* expSetting = direction == HFSwipeDirectionLeftToRight ? _leftExpansion : _rightExpansion;
    ...
    if(!_activeExpansion && expSetting.fillOnTrigger) {
        ...
        if (buttonsView) {
            __weak HFSwipeButtonsView * expansionView = direction == HFSwipeDirectionLeftToRight ? _leftView : _rightView;
            __weak HFSwipeTableCell * weakself = self;
...
- (void)setSwipeOffset:(CGFloat)offset animated:(BOOL)animated completion:(void(^)(BOOL finished))completion {
    HFSwipeAnimation *animation = animated ? [[HFSwipeAnimation alloc] init] : nil;
    [self setSwipeOffset:offset animation:animation completion:completion];
}

- (void)setSwipeOffset:(CGFloat)offset animation:(HFSwipeAnimation *)animation completion:(void(^)(BOOL finished))completion {
    if (_displayLink) {
        [_displayLink invalidate];
        _displayLink = nil;
    }
...
- (CGFloat)filterSwipe:(CGFloat)offset {
    ...
    else if (!_allowsOppositeSwipe && _firstSwipeState == HFSwipeStateSwipingLeftToRight && offset < 0) {
        offset = 0;
    }
    else if (!_allowsOppositeSwipe && _firstSwipeState == HFSwipeStateSwipingRightToLeft && offset > 0 ) {
        offset = 0;
    }
    return offset;
}
...
- (void)panHandler:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (_swipeOffset != 0) {
            _firstSwipeState = _swipeOffset > 0 ? HFSwipeStateSwipingLeftToRight : HFSwipeStateSwipingRightToLeft;
        }
        
        if (!_allowsMultipleSwipe) {
            NSArray * cells = [self parentTable].visibleCells;
            for (HFSwipeTableCell * cell in cells) {
                if ([cell isKindOfClass:[HFSwipeTableCell class]] && cell != self) {
        ...
        if (_firstSwipeState == HFSwipeStateNone) {
            _firstSwipeState = offset > 0 ? HFSwipeStateSwipingLeftToRight : HFSwipeStateSwipingRightToLeft;
        }
        self.swipeOffset = [self filterSwipe:offset];
    }
    else {
        __weak HFSwipeButtonsView * expansion = _activeExpansion;
        if (expansion) {
            __weak UIView * expandedButton = [expansion getExpandedButton];
            HFSwipeExpansionSettings * expSettings = _swipeOffset > 0 ? _leftExpansion : _rightExpansion;
            ...
            HFSwipeSettings * settings = _swipeOffset > 0 ? _leftSwipeSettings : _rightSwipeSettings;
            HFSwipeAnimation * animation = nil;
            if (_targetOffset == 0) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer == _panRecognizer) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(swipeTableCell:canSwipe:fromPoint:)]) {
            ...
            _allowSwipeLeftToRight = [_delegate swipeTableCell:self canSwipe:HFSwipeDirectionLeftToRight fromPoint:point];
            _allowSwipeRightToLeft = [_delegate swipeTableCell:self canSwipe:HFSwipeDirectionRightToLeft fromPoint:point];
        }
        else if (_delegate && [_delegate respondsToSelector:@selector(swipeTableCell:canSwipe:)]) {
            #pragma clang diagnostic push
            #pragma clang diagnostic ignored "-Wdeprecated-declarations"
            _allowSwipeLeftToRight = [_delegate swipeTableCell:self canSwipe:HFSwipeDirectionLeftToRight];
            _allowSwipeRightToLeft = [_delegate swipeTableCell:self canSwipe:HFSwipeDirectionRightToLeft];
            #pragma clang diagnostic pop
        }
        else {
            [self fetchButtonsIfNeeded];
