...
#define HFMainThreadAssert() NSAssert([NSThread isMainThread], @"HFProgressHUD needs to be accessed on the main thread.");

CGFloat const HFProgressMaxOffset = 1000000.f;

static const CGFloat HFDefaultPadding = 4.f;
static const CGFloat HFDefaultLabelFontSize = 16.f;
static const CGFloat HFDefaultDetailsLabelFontSize = 12.f;


@interface HFProgressHUD ()
...
@interface HFProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    HFProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    HFProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (HFProgressHUD *)HUDForView:(UIView *)view {
...
- (void)commonInit {
    ...
    _animationType = HFProgressHUDAnimationFade;
    _mode = HFProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(HFProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == HFProgressHUDAnimationZoom) {
        type = animatingIn ? HFProgressHUDAnimationZoomIn : HFProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == HFProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == HFProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == HFProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == HFProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    HFProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<HFProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    HFBackgroundView *backgroundView = [[HFBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = HFProgressHUDBackgroundStyleSolidColor;
    ...
    HFBackgroundView *bezelView = [HFBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:HFDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:HFDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [HFProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:HFDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[HFRoundProgressView class]];

    HFProgressHUDMode mode = self.mode;
    if (mode == HFProgressHUDModeIndeterminate) {
    ...
    else if (mode == HFProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[HFBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HFProgressHUDModeDeterminate || mode == HFProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[HFRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == HFProgressHUDModeAnnularDeterminate) {
            [(HFRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == HFProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HFProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[HFProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[HFProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[HFRoundProgressView class]]) {
        HFRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [HFRoundProgressView appearanceWhenContainedIn:[HFProgressHUD class], nil];
#else
        appearance = [HFRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[HFProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((HFRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((HFRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[HFBarProgressView class]]) {
        HFBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [HFBarProgressView appearanceWhenContainedIn:[HFProgressHUD class], nil];
#else
        appearance = [HFBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[HFProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((HFBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((HFBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    HFBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoHFie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? HFDefaultPadding : 0.f;
...
- (void)setMode:(HFProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == HFProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuHFer >= kCFCoreFoundationVersionNuHFer_iOS_8_0;
...
@implementation HFBarProgressView
...
@implementation HFBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = HFProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(HFProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    HFProgressHUDBackgroundStyle style = self.style;
    if (style == HFProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == HFProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation HFProgressHUDRoundedButton
